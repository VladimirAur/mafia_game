...
const RolesItem = ({ role, id, resetRole, incRoleNumber,decRoleNumber}) => {
	const {name, number} = role;
	return (
		<li className="role">
			<div className="role__name">{name}</div>
			<div className="role__count">
				<button className="role__btn role__btn--left" onClick={() => incRoleNumber(id)}>-</button>
				<div className="role__count-number">{number}</div>
				<button className="role__btn role__btn--right" onClick={() => decRoleNumber(id)}>+</button>
